  ...
  return (
    <div className="space-y-6">
      <div onClick={() => fileInput.current?.click()} onDrop={(e) => { e.preventDefault(); handleFile(e.dataTransfer.files?.[0]); }} onDragOver={(e) => e.preventDefault()} className="cursor-pointer p-12 rounded-2xl border-2 border-dashed border-gray-300 dark:border-slate-700 hover:border-emerald-500 bg-gray-50 dark:bg-slate-900/50 text-center transition">
        ...
        {!file ? (<><div className="text-5xl mb-3">🚀</div><p className="text-lg font-medium mb-1">Drop image here or click to browse</p><p className="text-sm text-gray-500">JPG, PNG → WebP (typically 25-40% smaller)</p></>) : (<><div className="text-5xl mb-3">✅</div><p className="text-lg font-medium">{file.name}</p><p className="text-sm text-gray-500">{fmt(file.size)} · {file.type}</p></>)}
      </div>

      {file && (
        <div className="p-5 rounded-xl border border-gray-200 dark:border-slate-800">
          <label className="block text-sm font-medium mb-2">WebP Quality: <span className="font-mono">{Math.round(quality * 100)}%</span></label>
          ...
          <p className="text-xs text-gray-500 mt-2">85% is Google&apos;s recommended baseline for the web.</p>
        </div>
      )}
      ...
      {file && <button onClick={convert} disabled={busy} className="w-full py-3 px-6 rounded-lg bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-white font-medium transition">{busy ? 'Converting…' : 'Convert to WebP'}</button>}
      ...
      {out && file && (
        <div className="p-6 rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/20">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <h3 className="font-semibold text-lg mb-1">✅ Converted</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{fmt(file.size)} → <strong>{fmt(out.size)}</strong> ({reduction}% smaller)</p>
            </div>
            <button onClick={download} className="px-5 py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white font-medium transition">Download WebP</button>
          </div>
